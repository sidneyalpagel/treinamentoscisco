...
@section('conteudo')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-800">Editar treinamento</h1>
        <p class="text-sm text-slate-500">{{ $treinamento->titulo }}</p>
    </div>

    <form method="POST" action="{{ route('admin.treinamentos.update', $treinamento) }}">
        @csrf
        @method('PUT')
        @include('admin.treinamentos._form', ['textoBotao' => 'Salvar alterações'])
    </form>
@endsection
