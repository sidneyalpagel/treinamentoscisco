        ...
        <aside class="space-y-5">
            @if ($treinamento->modalidadeOnline() && $treinamento->temSala())
                <div class="rounded-xl border border-brand-200 bg-brand-50 shadow-sm p-5">
                    <div class="flex items-center gap-2 text-brand-900 font-semibold mb-1">
                    ...
                    <p class="text-sm text-brand-800/80 mb-4">Participe pelo navegador, sem instalar nada.</p>
                    <a href="{{ route('sala.publica', $treinamento->sala_codigo) }}" target="_blank"
                       class="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-brand-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-800 transition-colors">
                        Entrar na reunião
                    </a>
                </div>
            @endif
