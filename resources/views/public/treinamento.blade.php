                ...
                <div class="p-5 space-y-4">
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between gap-4">
                            ...
                            <dd class="font-medium text-slate-800">{{ $treinamento->vagas ?? 'Ilimitadas' }}</dd>
                        </div>
                    ...
                    @if ($treinamento->inscricoesAbertas())
                        <div class="rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-800">
                            <div class="flex items-center gap-2 font-medium">
                                <span class="w-2 h-2 rounded-full bg-emerald-500"></span> Inscrições abertas
                            </div>
                        </div>
                        <button type="button" disabled
                                class="w-full rounded-lg bg-brand-700/60 px-4 py-2.5 text-sm font-semibold text-white cursor-not-allowed">
                            Formulário de inscrição (em breve)
                        </button>
                        <p class="text-xs text-slate-400 text-center">O formulário de inscrição será habilitado no próximo módulo.</p>
                    @else
                        <div class="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-sm text-slate-600">
                            <div class="flex items-center gap-2 font-medium">
                                <span class="w-2 h-2 rounded-full bg-slate-400"></span> Inscrições indisponíveis
                            </div>
                        </div>
                    @endif
                </div>
